  ...
  let formData = {
    ...
    export async function clientesCancelados() {
      try {
        ...
        let clientesFiltrados = data.data.filter((client) => client.cancelado === true);
      ...
      <div class="flex items-center gap-x-3">
        ...
        {#if clientesCancelado}
          <span class="text-center w-24 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">
            {clientesCancelado.length} {clientesCancelado.length === 1 ? "cliente" : "clientes"}
          </span>
        {:else}
              ...
              <tr>
                  ...
                  <td class="px-4 py-4 text-sm whitespace-nowrap">
                    <div class="inline-flex items-center rounded-md shadow-sm">
                        <!--Editar usuario-->
                        <button class="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 text-slate-800 hover:text-blue-600 text-sm bg-white hover:bg-slate-100 border border-slate-200 rounded-l-lg font-medium px-4 py-2 inline-flex space-x-1 items-center">
