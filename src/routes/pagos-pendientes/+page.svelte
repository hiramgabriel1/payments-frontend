...
/*   const calcularFechaLimite = () => {
  ...
  export async function clientesPendientes() {
    try {
      const clients = await fetch("https://prestamista-backend.onrender.com/api/users/pending-payments");
      let data = await clients.json();
      clientesPendiente = data.data;
      loading = false;
    } catch (error) {
  ...
  async function actualizarUser() {
    try {
      patchUser = await fetch(
        `https://prestamista-backend.onrender.com/api/update/user/${idUser}`,
        {
          method: "PATCH",
  ...
  export async function deleteClientsPendientes(idDelete) {
    const response = await fetch(
      `https://prestamista-backend.onrender.com/api/delete/user/${idDelete}`,
      {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      }
