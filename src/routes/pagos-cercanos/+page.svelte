<script>
  ...
  async function getClients() {
    try {
      ...
      const sortedClients = compararPorFechaPago(data.data);
      dataClients = sortedClients;
      loading = false;
  ...
  function compararPorFechaPago(clients) {
    return clients.slice().sort((clienteA, clienteB) => {
      ...
      const fechaPrestamoB = parseFecha(clienteB.fechaPrestamo);
      const fechaPagoA = parseFecha(clienteA.fechaPago);
      const fechaPagoB = parseFecha(clienteB.fechaPago);

      const diferenciaA = fechaPagoA - fechaPrestamoA;
      const diferenciaB = fechaPagoB - fechaPrestamoB;
